...
BEGIN
    -- Start transaction to ensure all or nothing commits
    BEGIN
        -- Get player data
        SELECT 
        ...
        IF v_player.cash < v_total_price THEN
            RAISE NOTICE 'Not enough cash. Need %, have %', v_total_price, v_player.cash;
            ROLLBACK;
            RETURN FALSE;
        END IF;
        ...
        IF v_available_space < v_space_required THEN
            RAISE NOTICE 'Not enough inventory space. Need %, have %', v_space_required, v_available_space;
            ROLLBACK;
            RETURN FALSE;
        END IF;
        ...
        IF v_market_item.quantity < p_quantity THEN
            RAISE NOTICE 'Not enough stock. Need %, have %', p_quantity, v_market_item.quantity;
            ROLLBACK;
            RETURN FALSE;
        END IF;
        
        -- Reduce player's cash - THIS IS THE CRITICAL UPDATE
        UPDATE players
        SET 
            cash = cash - v_total_price,
            inventory_count = COALESCE(inventory_count, 0) + p_quantity
        ...
        INSERT INTO player_inventory (
            ...
            NOW(), NOW(),
            v_market_item.current_price -- Use purchase price as estimated value initially
        );
    ...
    EXCEPTION WHEN OTHERS THEN
        -- Log the error
        RAISE NOTICE 'Error in buy_record: %', SQLERRM;
        ROLLBACK;
        v_success := FALSE;
    END;
