...
const Home = () => {
  const { user, playInstantly } = useAuth();
  ...
  const handlePlayInstantly = async () => {
    ...
    try {
      ...
      console.error('Error playing instantly:', error);
    } finally {
      setLoading(false);
    }
  };
              ...
              {user ? (
                ...
                <>
                  <button
                    ...
                    disabled={loading}
                    className="px-8 py-3 rounded-md bg-green-600 text-white font-medium text-lg hover:bg-green-700 transition-colors w-full sm:w-auto flex items-center justify-center"
                  >
                    {loading ? (
